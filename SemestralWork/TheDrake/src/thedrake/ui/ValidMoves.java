package thedrake.ui;

import thedrake.models.boards.*;
import thedrake.models.moves.interfaces.*;
import thedrake.models.moves.stacks.*;
import thedrake.models.positions.boards.*;
import thedrake.models.positions.factories.*;
import thedrake.models.positions.interfaces.*;
import thedrake.models.states.*;
import thedrake.models.tiles.interfaces.*;
import thedrake.models.tiles.troops.*;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

public class ValidMoves {
    private final GameState state;

    public ValidMoves(GameState state) {
        this.state = state;
    }

    public List<Move> boardMoves(BoardPos position) {
        if (this.state.armyOnTurn().boardTroops().isPlacingGuards())
            return Collections.emptyList();

        Tile tile = this.state.tileAt(position);
        if (tile.hasTroop()) {
            if (((TroopTile) tile).side() != this.state.sideOnTurn())
                return Collections.emptyList();

            return ((TroopTile) tile).movesFrom(position, this.state);
        }

        return Collections.emptyList();
    }

    public List<Move> movesFromStack() {
        List<Move> moves = new ArrayList<Move>();
        PositionFactory pf = this.state.board().positionFactory();
        Army armyOnTurn = this.state.armyOnTurn();
        if (!armyOnTurn.boardTroops().isLeaderPlaced()) {
            int j = 0;
            if (this.state.sideOnTurn() == PlayingSide.ORANGE)
                j = this.state.board().dimension() - 1;

            for (int i = 0; i < this.state.board().dimension(); i++)
                moves.add(new PlaceFromStack(pf.pos(i, j)));
        } else if (armyOnTurn.boardTroops().isPlacingGuards()) {
            TilePos leader = armyOnTurn.boardTroops().leaderPosition();
            TilePos target = leader.step(0, 1);
            if (this.state.canPlaceFromStack(target))
                moves.add(new PlaceFromStack((BoardPos) target));

            target = leader.step(0, -1);
            if (this.state.canPlaceFromStack(target))
                moves.add(new PlaceFromStack((BoardPos) target));

            target = leader.step(1, 0);
            if (this.state.canPlaceFromStack(target))
                moves.add(new PlaceFromStack((BoardPos) target));

            target = leader.step(-1, 0);
            if (this.state.canPlaceFromStack(target))
                moves.add(new PlaceFromStack((BoardPos) target));
        } else {
            for (BoardPos pos : armyOnTurn.boardTroops().troopPositions()) {
                List<BoardPos> neighbours = pos.neighbours();
                for (BoardPos target : neighbours)
                    if (this.state.canPlaceFromStack(target))
                        moves.add(new PlaceFromStack(target));
            }
        }

        return moves;
    }

    public List<Move> allMoves() {
        List<Move> moves = new ArrayList<>();
        for (BoardPos pos : this.state.armyOnTurn().boardTroops().troopPositions())
            moves.addAll(boardMoves(pos));

        moves.addAll(movesFromStack());
        return moves;
    }
}
